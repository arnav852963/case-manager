...
const getServiceRecordByCaseId = asyncHandler(async (req, res) => {
    ...
    if (!serviceRecords || serviceRecords.length === 0) throw new ApiError(500, "No service records found for this case");
...
const addNoteToServiceRecord = asyncHandler(async (req, res) => {
    ...
    const {note} = req.body;
    if (!recordId || !note.trim()) throw new ApiError(400, "Record ID and note are required");
